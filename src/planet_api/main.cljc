(ns planet_api.main
  "Kotodama WASM entrypoint for the Planet_api clean-room actor (L4) — Clojure port.
  ...
  facts (`planet_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "planet_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"planet_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "planet_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :planet_api-compat/ready)
